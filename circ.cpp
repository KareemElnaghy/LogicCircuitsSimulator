...
void circ::calculateDelay() {
  ...
  for (int i = 0; i < gates.size(); i++) {
    ...
    while (!tempTimes.empty()) {
      ...
      if (index > 0 && (gates[i].get_inputs()[index].timeStamp - gates[i].get_inputs()[index - 1].timeStamp) < gates[i].get_delay_ps() && (gates[i].get_inputs()[index].timeStamp - gates[i].get_inputs()[index - 1].timeStamp) != 0) {
